...
def _bonded_geometry(atoms, extra_bonds: list[tuple[int, int]] | None = None):
    ...
    residue_types = _template_residue_types()
...
def _case1_topology_sections(atoms, extra_bonds: list[tuple[int, int]] | None = None):
    bonds, angles, graph_dihedrals, _improper_dihedrals = _bonded_geometry(atoms, extra_bonds)
...
def _write_top4scm(path: Path, atoms, extra_bonds: list[tuple[int, int]] | None = None):
    resnums = _smog2_residue_numbers(atoms)
    resnames = _smog2_residue_names(atoms)
    ordered_bonds, angles, dihedral_rows = _case1_topology_sections(atoms, extra_bonds)
    lines = [
...
def _write_case1_final_top(
    ...
    *,
    ...
    contact_bond_specs = _bond_contact_specs()
...
def _generate_contacts_with_scm(
    ...
    *,
    ...
    _write_top4scm(scm_top, atoms, extra_bonds)
    cmd = [
        java, "-jar", str(scm),
        "-g", str(scm_gro),
        "-freecoor",
...
def main(argv: list[str]) -> int:
    ...
    if ns.userContacts:
        ...
        contacts = _generate_contacts_with_scm(coord, top, contacts_path, atoms, extra_bonds=aa_extra_bonds) or []
    elif gaussian:
